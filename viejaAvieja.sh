...
function crearBackupInfraNueva {
    ...
    mysqldump -u root -p$PASS --single-transaction --no-create-info --compact $BASE > $BACKUP/haciaNueva/$BASE.sql    
}
    ...
    case "${FLAG}" in
        ...
        p)
            ...
            if [ $RESPUESTA == y ]; then
                ...
                echo "No se realizó el Backup, la base ingresada no existe o seleccionó la opción NO."
            fi
        ;;
        *)
